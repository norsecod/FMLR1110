...
LORA_BASICS_MODEM := lbm
APPBUILD_ROOT = build
APPTARGET_ROOT = $(PROJECT)

include $(BSP)/makefiles/board.mk

ifeq ($(RADIO),lr1110)
-include extensions/makefiles/lr11xx.mk
endif
...
ifeq ($(RADIO),sx1261)
-include extensions/makefiles/sx126x.mk
endif
...
ifeq ($(RADIO),sx1268)
-include extensions/makefiles/sx1268.mk
endif

ifeq ($(RADIO),sx128x)
-include extensions/makefiles/sx128x.mk
endif

#-----------------------------------------------------------------------------
# Define target build directory
#-----------------------------------------------------------------------------
TARGET_MODEM = $(APPTARGET_ROOT)
BUILD_DIR_MODEM = $(APPBUILD_ROOT)
...
CFLAGS += -fno-builtin $(MCU_FLAGS) $(BOARD_C_DEFS) $(USER_APP_C_DEFS) $(COMMON_C_DEFS) $(MODEM_C_DEFS) $(BOARD_C_INCLUDES) $(USER_APP_C_INCLUDES) $(COMMON_C_INCLUDES) $(MODEM_C_INCLUDES) $(OPT) $(WFLAG) -MMD -MP -MF"$(@:%.o=%.d)"
...
application: $(BUILD_DIR_MODEM)/$(TARGET_MODEM).elf $(BUILD_DIR_MODEM)/$(TARGET_MODEM).hex $(BUILD_DIR_MODEM)/$(TARGET_MODEM).bin
	$(call success,$@)
...
$(BASIC_MODEM_LIB):
	$(MAKE) -C $(LORA_BASICS_MODEM) basic_modem REGION=$(REGION) RADIO=$(RADIO) MCU_FLAGS="$(MCU_FLAGS)" MODEM_TRACE="$(MODEM_TRACE)" $(MTHREAD_FLAG)
